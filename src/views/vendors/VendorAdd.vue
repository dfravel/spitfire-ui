        ...
        <div class="bg-white shadow-md rounded my-6">
            <div class="flex flex-row bg-gray-200">
                <div class="w-1/3 my-4 text-left px-8">
                    <div class="text-2xl">Add a Vendor</div>
                    <div class="text-sm">
                        Please enter the vendor information
                    </div>
                </div>
...
export default {
    data() {
        ...
        return {
            vendorTypes,
            vendor: this.createFreshVendorObject()
        };
    },
    validations: {
        vendor: {
    ...
    methods: {
        ...
        handleSubmit() {
            ...
            if (!this.$v.$invalid) {
                this.$store
                    .dispatch("vendor/post", this.vendor)
                    .then(response => {
                        this.$log.info(
                            `success! we've successfully submitted the vendor ${JSON.stringify(response.data)}`
                        );
                        this.$router.push({
                            path: `/vendors`
                        });
